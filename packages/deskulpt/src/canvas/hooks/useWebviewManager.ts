...
export function useWebviewManager() {
  ...
  const createWebview = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        const existingWebview = await Webview.getByLabel(`widget-${id}`);
        if (existingWebview) {
        ...
        await commands.core.createWidgetWebview(
          id,
          settings.x,
          settings.y,
          settings.width,
        ...
        const webview = await Webview.getByLabel(`widget-${id}`);
  ...
  useEffect(() => {
    const promises = widgetIds.map(async (id) => {
      let existingWebview = await Webview.getByLabel(`widget-${id}`);
      if (!existingWebview) {
