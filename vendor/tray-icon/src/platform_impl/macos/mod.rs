...
use crate::icon::Icon;
use crate::{
    menu, Error, MouseButton, MouseButtonState, Rect, TrayIconAttributes, TrayIconEvent, TrayIconId,
};
    ...
    fn create(
        ...
        let tray_target = unsafe {
            ...
            let target = mtm.alloc().set_ivars(TrayTargetIvars {
                ...
                control_click_active: Cell::new(false),
            });
...
struct TrayTargetIvars {
    ...
    control_click_active: Cell<bool>,
}
...
define_class!(
    ...
    impl TrayTarget {
        ...
        fn on_mouse_down(&self, event: &NSEvent) {
            ...
            let is_control_click = modifier_flags.contains(NSEventModifierFlags::Control);
            self.ivars().control_click_active.set(is_control_click);
            let button = if is_control_click {
                MouseButton::Right
            } else {
                MouseButton::Left
            };
        ...
        fn on_mouse_up(&self, event: &NSEvent) {
            ...
            let button = if self.ivars().control_click_active.replace(false) {
                MouseButton::Right
            } else {
                MouseButton::Left
            };
