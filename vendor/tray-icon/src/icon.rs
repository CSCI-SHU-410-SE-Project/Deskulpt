    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            BadIcon::DimensionsVsPixelCount {
                ...
                f,
                "The specified dimensions ({:?}x{:?}) don't match the number of pixels supplied \
                 by the `rgba` argument ({:?}). For those dimensions, the expected pixel count is \
                 {:?}.",
...
impl Icon {
    ...
    /// equal `rgba.len() / 4`. Otherwise, this will return a `BadIcon`
    /// error.
    pub fn from_rgba(rgba: Vec<u8>, width: u32, height: u32) -> Result<Self, BadIcon> {
        Ok(Icon {
