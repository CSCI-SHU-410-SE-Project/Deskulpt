...
4. Install node dependencies. Note that Rust dependencies should be installed
   ...
   Alternatively, you may not only want to install dependencies of the Deskulpt core,
   but also install dependencies and build the tooling packages. You can do this by
   running:
...
9. Develop on your new branch, using Git for version control. You may be interested in
   [developers' API references](/development/references) which provide details about the
...
- **Formatting**: Run the formatter to ensure that your code is properly formatted, in
  ...
  npm run format -- --fix   # Automatic fix if possible
  npm run format js         # Lint only the frontend
  npm run format rs         # Lint only the backend
  npm run format -- --help  # See more details
  ```
...
- **Linting**: Run the linters to check for code styling issues. This is useful to
  ...
  ```bash
  npm run lint            # Check
  npm run lint -- --fix   # Automatic fix if possible
  npm run lint js         # Lint only the frontend
  npm run lint rs         # Lint only the backend
  npm run lint -- --help  # See more details
  ```
...
- **Testing**: Run the test suite to ensure that your changes do not break existing
  ...
  ```bash
  npm run test rs           # Run all backend tests
  npm run test rs bundler   # Run only backend tests that match the "bundler" pattern
  npm run test rs -- --cov  # Also generate coverage report
  npm run test -- --help    # See more details
  ```
